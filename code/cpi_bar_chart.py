...
if unit_price_data is None or unit_price_data.empty:
    ...
    # 保存图表
    cpi_chart_filename = os.path.join(output_dir, "cpi_bar_chart.png")
    plt.savefig(cpi_chart_filename, dpi=300, bbox_inches="tight")
    plt.close()

    print(f"租房性价比图表已成功保存到 {cpi_chart_filename}")
