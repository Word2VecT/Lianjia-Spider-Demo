...
if unit_price_data is None or unit_price_data.empty:
    ...
    mean_rent["性价比指数 (CPI)"] = mean_rent["人均 GDP（元）"] / mean_rent["单位面积租金（元/月/㎡）"]
    ...
    scatter = sns.scatterplot(
        ...
        y="人均 GDP（元）",  # 反转 Y 轴为 GDP
        size="性价比指数 (CPI)",
        hue="城市",
        palette=[city_colors[city] for city in mean_rent_sorted["城市"]],
        alpha=0.7,
        edgecolor="w",
    ...
    plt.title("五个城市单位面积租金与人均 GDP 的关系及租房性价比 (CPI)", fontsize=16, fontweight="bold", pad=20)
    ...
    # 保存图表
    scatter_chart_filename = os.path.join(output_dir, "gdp_unit_price_scatter_chart.png")
    plt.savefig(scatter_chart_filename, dpi=300, bbox_inches="tight")
    plt.close()

    print(f"优化后的气泡图已成功保存到 {scatter_chart_filename}")
